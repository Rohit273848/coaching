...
import { Calendar, Clock, CheckCircle2, ArrowRight, Star, ShieldCheck, Users } from 'lucide-react';
...
export default function CourseCard({ course, onOpenEnquiry }) {
  return (
    <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-saarthi hover:shadow-saarthi-lg transition-all flex flex-col h-full group hover:-translate-y-1">
      {/* Top Banner Tag */}
      <div className="bg-[#1D2B53] text-white p-4 flex items-center justify-between border-b border-[#283C6E]">
        <div className="flex items-center gap-2">
          <span className="bg-[#1E4FA8] text-white text-[10px] font-black px-2.5 py-0.5 rounded-full tracking-wider uppercase">
            {course.category}
          </span>
          {course.badge && (
            <span className="bg-[#F8B81F] text-[#1D2B53] text-[10px] font-black px-2 py-0.5 rounded-full flex items-center gap-1">
        ...
        <span className="text-xs text-slate-300 font-bold flex items-center gap-1">
          <Clock className="w-3.5 h-3.5 text-[#F8B81F]" />
          {course.duration}
        </span>
      </div>
      ...
      <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between space-y-4">
        <div>
          ...
          <div className="space-y-1 mb-2.5">
            <span className="text-[11px] font-bold text-[#F8941F] uppercase tracking-wide">
        ...
        <div className="pt-3 border-t border-slate-100 space-y-3">
          <div className="flex items-center justify-between text-xs text-slate-600 font-medium">
            <div className="flex items-center gap-1.5">
              ...
              <span>Starts: <strong className="text-slate-900">{course.batchStartDate}</strong></span>
            </div>
            <span className="text-slate-500 font-bold">{course.studentCount}</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            ...
            <button
              ...
              className="bg-[#1E4FA8] hover:bg-[#163E85] active:scale-[0.98] text-white font-black text-xs py-3 px-3 rounded-xl text-center transition-all shadow-xs min-h-[44px] flex items-center justify-center cursor-pointer"
            >
              Enquire Batch
            </button>
          </div>
